...
public:
	...
	{
		// @TODO if this is a construtor, include effect of base constructors
		// @TODO include the effects of modifiers
		// Both of these cannot be done in visiting order!
...
map<ASTNode const*, CallgraphNode> CallgraphBuilder::build(vector<shared_ptr<ASTNode>> const& _nodes)
{
	CallgraphBuilderVisitor v;

	for (auto const& node: _nodes)
		node->accept(v);

	solAssert(!v.nodes().count(nullptr), "Invalid pointer inserted");
